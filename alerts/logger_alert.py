"""Alerts via logging (console + file) with optional Slack webhook notifications."""
...
# Slack POST timeout — keep short so a slow Slack never blocks the collector
_SLACK_TIMEOUT = aiohttp.ClientTimeout(total=5)

# Only escalate WS reconnect attempts to Slack once this many consecutive
# failures have occurred (avoids noise from single transient disconnects)
_WS_RECONNECT_SLACK_THRESHOLD = 3


class LoggerAlert:
    """Alert dispatcher: always logs to the standard logger; optionally posts to Slack."""
    ...
    async def prop_opportunity(self, opportunities: list) -> None:
        ...
        if not self._slack_url or not opportunities:
            return
